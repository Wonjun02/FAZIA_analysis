void do_PID()
{
  ...
  for( int i=0; i<4; i++ )
    for( int j=0; j<4; j++ )
    {
      ...
      histProj[i][j] = new TH1D(Form("histProj_q%d_t%d", qSel, tSel),"histProj", 150, 50, 200);
      histDiff[i][j] = new TH1D(Form("histDiff%d_t%d", qSel, tSel),"histDiff", 150, -20, 50);
      ...
      gPad->SetLogz();
      h2->GetXaxis()->SetRangeUser( 2, 80 );
      h2->GetYaxis()->SetRangeUser( 0.2, 40 );
      h2->Draw("colz");
      c->Update();
      ...
      for( int ix=0; ix<nx; ix++ )
      {
        ...
        for( int iy=0; iy<ny; iy++ )
        {
          ...
          if( std::abs(diff) < compareVal )
          {
            ...
            int idx = histProj[i][j]->FindBin( xx );

            // Then accumulate the histogram
            double val = histProj[i][j]->GetBinContent( idx ) + counts;
            histProj[i][j]->SetBinContent( idx, val );
          }
        }
      }
    ...
    auto outputFile = new TFile( Form("data_%dMeV_1.root", En), "RECREATE" );
  for( int i=0; i<4; i++ )
    for( int j=0; j<4; j++ )
    {
      fTG[i][j]->Write();
